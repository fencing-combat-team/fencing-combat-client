...
public class MainMenuEvent : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        btnStart.onClick.AddListener(() => { SceneManager.LoadScene("Room"); });

        Button btnSetting = GameObject.Find("btnSetting").GetComponent<Button>();
        btnSetting.onClick.AddListener(() => { isSettingsOpening = true; });

        Button btnQuit = GameObject.Find("btnQuit").GetComponent<Button>();
        btnQuit.onClick.AddListener(() => { Application.Quit(0); });
    ...
    void OpenSettings()
    {
        Vector3 vectorSetting = new Vector3(panelSetting.transform.position.x, panelSetting.transform.position.y - 0.1f,
            panelSetting.transform.position.z);
    ...
    void BackGroundShow()
    {
        ...
        float x;
        if (transformParachute.position.x + 0.005f * direction >= -3 ||
            transformParachute.position.x + 0.005f * direction <= -6)
        {
